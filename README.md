...
- 光點從分散的外圈區域起步，沿寬廣弧線游移；靠近時局部隨機互斥或短暫吸引纏繞，每點同時最多一位夥伴，互動後有分離空檔，並時而向更外圈彈出。個別角度擾動依牌數限制，避免其他光點一同擠入配對；每道光保留同色、逐漸變細淡出的真實路徑彗星尾光
- 景深距離逐步增加至遠方，外圈軌跡適度展寬以保留活動範圍，不再讓整群隨透視集中到中央；光頭和尾光仍以真實透視縮小、微微變淡。慢載入時繼續在遠處游移，不突然跳回近景；歸位承接最後的遠方位置、大小和亮度，尾光在成牌前褪去
...
- `RELEASE-20260915-02.md`：大弧線遠離、深度透視與遠方歸位銜接
- `RELEASE-20260915-03.md`：分散外圈起步、局部隨機纏繞／互斥、外彈與防聚團
